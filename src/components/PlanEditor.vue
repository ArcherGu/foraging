<template>
  <div class="plan-editor" h="full" w="full" p="6">
      ...
      <div max="h-8/12" overflow="x-auto" ref="itemListRef">
        ...
        >
          ...
          <div w="10" position="flex" items="center" justify="center">
            <button class="btn xs-btn no-shadow" bg="pink-500" @click="deleteItem(index)">
              <uil-times />
            </button>
          </div>
        </div>
    ...
    <div position="bottom-0 left-0 right-0 flex" w="full" items="center" justify="center" m="t-10">
      <button class="btn round-btn sm-btn no-shadow" bg="light-blue-400" m="r-5" @click="confirm">
        <uil-check />
      </button>
...
import { watch, ref, toRaw, reactive, nextTick } from 'vue'
...
const emit = defineEmits(['delete', 'cancel', 'confirm'])

const typeOptions = ['Breakfast', 'Lunch', 'Dinner', 'Afternoon Tea', 'Supper', 'Dessert', 'Other']
